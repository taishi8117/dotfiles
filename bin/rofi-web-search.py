...
def print_prefix_list():
    ...
    for key in keys:
        print(f'!!-- {key.rjust(3)}: {PREFIX[key][0]}')


def check_prefix(search_string):
    for key in PREFIX.keys():
        if search_string.startswith(f'{key} '):
            return True
    return False


def handle_prefix(search_string):
    # defaults to ddg
    search_url = PREFIX['d']
    term = ''
    ...
    for key in PREFIX.keys():
        if search_string.startswith(f'{key} '):
            search_url = PREFIX[key][1]
            term = search_string[len(key) + 1:]
            break

    url = search_url + urllib.parse.quote_plus(term)
...
def main():
    ...
    if check_prefix(search_string):
        handle_prefix(search_string)
        ...
        print('!!-- Close with "!" to get search suggestions')
        print_prefix_list()
    else:
